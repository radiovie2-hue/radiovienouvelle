...
export default function TeamCarousel({ cards }) {
  ...
  const chevron = (direction) => (
    <svg className="tc-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="team-carousel-container">
      <div className="tc-layout-wrapper">
        <div ref={containerRef} className="tc-fan-layout">
          {cards.map((card, index) => {
            const image = (
              <div className="tc-img-wrapper">
                {card.imgUrl ? (
                  <img src={card.imgUrl} loading="lazy" alt={card.name || `Card ${index}`} className="tc-img" />
                ) : (
                ...
                <div className="tc-card-content">
                  ...
                  <p className="tc-role">{card.role}</p>
                </div>
              </div>
            );
            return (
              <div key={index} className="fan-card tc-card">{image}</div>
            );
          })}
        </div>
      </div>
      ...
      {needsPagination && (
        <div className="tc-pagination">
          <button className="tc-arrow" onClick={() => cycle("left")} aria-label="Previous">
            {chevron("left")}
          </button>
          <div className="tc-dots">
            {cards.map((_, i) => (
              <span key={i} className={`tc-dot ${i === centerIndex ? "active" : ""}`} />
            ))}
          </div>
          <button className="tc-arrow" onClick={() => cycle("right")} aria-label="Next">
            {chevron("right")}
          </button>
        </div>
      )}
